...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        self.pred_trajs = {}
        if 'NFS' in root:
            self.img_names = [os.path.join(root, x.split('30/')[1]) for x in img_names]
        elif 'TB' in root:
            ...
            if os.path.exists(traj_file):
                ...
                with open(traj_file, 'r') as f :
                    for x in f.readlines():
                        row_rs = x.strip().split('\t')
                        if len(row_rs)==1:
                            row_rs = list(map(float,row_rs[0].split(',')))
                        else:
                            row_rs = list(map(float, row_rs))
                        pred_traj.append(row_rs)
