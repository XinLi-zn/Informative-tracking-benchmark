...
class NUSPROVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        super(NUSPROVideo, self).__init__(name, root, video_dir,
...
class NUSPRODataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(NUSPRODataset, self).__init__(name, dataset_root)
        ...
        if os.path.isfile(json_path):
            ...
            for video in videos:
                self.videos[video] = NUSPROVideo(video,
                                                 dataset_root,
            ...
            print('NUS-PRO dataset, number of sequence:', len(videos))

            for video in videos:
                init_rect_ = np.loadtxt(os.path.join(dataset_root, video, 'groundtruth.txt'), delimiter=' ')
                init_rect_[:, 2:] = init_rect_[:, 2:] - init_rect_[:, 0:2] + 1
                init_rect_ = list(init_rect_)
                init_rect = init_rect_#[[None]] * 10000
                init_rect[0] = init_rect_[0]
                gt_rect = init_rect
                _, img_names = self.get_fileNames(os.path.join(dataset_root, video))
